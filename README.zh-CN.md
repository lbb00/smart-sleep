...
1. **检测外接显示器** — 使用 `ioreg` 结合盖子状态进行快速准确检测
...
Homebrew 现在只负责安装可执行文件。需要你手动运行 `smart-sleep install` 来配置 `sudoers` 和 LaunchAgent。
...
如果手动写入了非法值，守护进程会忽略该值并在日志中记录，不会破坏正在运行的服务。
